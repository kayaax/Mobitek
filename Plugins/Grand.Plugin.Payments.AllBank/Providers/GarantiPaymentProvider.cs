        ...
        public async Task<PaymentGatewayResult> ThreeDGatewayRequest(PaymentGatewayRequest request)
        {
            try
            {
                ...
                if (!request.CommonPaymentPage)
                {
                    ...
                    parameters.Add("cardexpiredatemonth", AllBankHelper.EncodeExpireMonth(request.ExpireMonth));//kart bitiş ay'ı
                    parameters.Add("cardexpiredateyear", request.ExpireYear);//kart bitiş yıl'ı
                }

                parameters.Add("secure3dsecuritylevel", secure3dSecurityLevel);//SMS onaylı ödeme modeli 3DPay olarak adlandırılıyor.
