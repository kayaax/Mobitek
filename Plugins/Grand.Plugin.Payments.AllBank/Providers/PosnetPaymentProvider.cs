...
namespace Grand.Plugin.Payments.AllBank.Providers
{
    public class PosnetPaymentProvider : IPaymentProvider
    {
        ...
        {
            string merchantId = request.BankParameters[nameof(YapıKrediBankModel.MerchantId)];
            string terminalId = request.BankParameters[nameof(YapıKrediBankModel.TerminalId)];
            string posnetId = request.BankParameters[nameof(YapıKrediBankModel.PosNetId)];
            ...
            try
            {
                ...
                string requestXml = $@"<?xml version=""1.0"" encoding=""utf-8""?>
                                        <posnetRequest>
                                            ...
                                            <oosRequestData>
                                                ...
                                                <expDate>{AllBankHelper.EncodeExpireMonth(request.ExpireMonth)}{request.ExpireYear}</expDate>
                ...
                var response = await _client.PostAsync(request.BankParameters[nameof(YapıKrediBankModel.VerifyUrl)], new FormUrlEncodedContent(httpParameters));
                ...
                return PaymentGatewayResult.Successed(parameters, request.BankParameters[nameof(YapıKrediBankModel.GatewayUrl)]);
            }
            catch (Exception ex)
            {
        ...
        public async Task<VerifyGatewayResult> VerifyGateway(VerifyGatewayRequest request, PaymentGatewayRequest gatewayRequest, IFormCollection form)
        {
            ...
            return VerifyGatewayResult.Successed(form["HostLogKey"], $"{form["HostLogKey"]}-{form["AuthCode"]}",
                ...
                form["ApprovedCode"], amount: xmlDocument.SelectSingleNode("posnetResponse/amount")?.InnerText);
        }
