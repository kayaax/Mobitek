    ...
    public class VakifBankPaymentProvider : IPaymentProvider
    {
        ...
        public async Task<PaymentGatewayResult> ThreeDGatewayRequest(PaymentGatewayRequest request)
        {
            try
            {
                ...
                string merchantId = request.BankParameters[nameof(VakıfBankModel.MerchantId)];
                string merchantPassword = request.BankParameters[nameof(VakıfBankModel.Password)];
                string enrollmentUrl = request.BankParameters[nameof(VakıfBankModel.EnrollmentUrl)];

                var httpParameters = new Dictionary<string, string>();
                httpParameters.Add("Pan", request.CardNumber);
                httpParameters.Add("ExpiryDate", $"{request.ExpireYear}{AllBankHelper.EncodeExpireMonth(request.ExpireMonth)}");
                httpParameters.Add("PurchaseAmount", request.TotalAmount.ToString("0:##", new CultureInfo("en-US")));
                httpParameters.Add("Currency", CurrencyCodes[request.CurrencyIsoCode]);//TL 949 | EURO 978 | Dolar 840
                ...
                */
                httpParameters.Add("BrandName", CardAssociations[request.CardAssociation]);
        ...
        public async Task<VerifyGatewayResult> VerifyGateway(VerifyGatewayRequest request, PaymentGatewayRequest gatewayRequest, IFormCollection form)
        {
            ...
            string merchantId = request.BankParameters[nameof(VakıfBankModel.MerchantId)];
            string merchantPassword = request.BankParameters[nameof(VakıfBankModel.Password)];
            string terminalNo = request.BankParameters[nameof(VakıfBankModel.TerminalNo)];
            ...
            var response = await _client.PostAsync(request.BankParameters[nameof(VakıfBankModel.VerifyUrl)], new FormUrlEncodedContent(parameters));
            string responseContent = await response.Content.ReadAsStringAsync();
            ...
            var amount = xmlDocument.SelectSingleNode("VposResponse/CurrencyAmount");
            var amountTl = xmlDocument.SelectSingleNode("VposResponse/TLAmount");
            ...
            var amountPay = amountTl == null ? amountTl : amount;
            ...
            return VerifyGatewayResult.Successed(transactionNode.InnerText, form["Xid"],
                ...
                resultCodeNode.InnerText, amount: amountPay.InnerText);
        }
